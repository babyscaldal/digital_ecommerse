...
export const getCommentsSingleProduct = createAsyncThunk(
  ...
  async (productId: number, thunkAPI) => {
    try {
      const response = await productServices.getCommentsSingleProduct(
        productId
      );
      return response.data;
    } catch (error) {
    ...
    filterProductsByRate: (state, action: PayloadAction<number[]>) => {
      if (action.payload.length) {
        state.filterProductsList = state.renderProducts.filter((product) => {
          return action.payload.includes(
            Number(product.rating.rate.toFixed(0))
          );
        });
      } else {
...
export const allProductsState = (state: RootState) =>
